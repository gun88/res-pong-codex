...
class Res_Pong_User_Service {
    ...
    public function delete_reservation() {
        ...
        $deleted = $this->repository->delete_reservation($id);
        $deleted ? wp_send_json_success() : wp_send_json_error();
    }

    public function find_users() {
        $results = $this->repository->find_users();
        return rest_ensure_response($results);
    }

    public function update_users_state($request) {
        $ids = $request->get_param('ids');
        $enabled = $request->get_param('enabled');
        if (!is_array($ids) || ($enabled !== '1' && $enabled !== '0')) {
            return new WP_REST_Response(['error' => 'Parametri non validi'], 400);
        }
        $this->repository->update_users_state($ids, $enabled);
    ...
    public function get_reservations($request) {
        ...
        $results = $this->repository->get_reservations_summary($start, $end);
        $grouped = Res_Pong_Common::group_reservations($results);
        $results = Res_Pong_Common::to_full_calendar(array_values($grouped), 'Gioco Libero', "#/reservations/");
        return rest_ensure_response($results);
    }
    ...
    public function _get_event_for_logged_user($event_id, int $user_id) {
    ...
    public function get_user_by_id($request) {
        ...
        return rest_ensure_response($this->repository->get_user_by_id($user_id));
    }

    public function delete_user_by_id($request) {
        $user_id = $request->get_param('user_id');
        return rest_ensure_response($this->repository->delete_user($user_id));
    }
    ...
    public function update_user($request) {
        ...
        if (!isset($params['user']) || !is_array($params['user'])) {
            return new WP_Error('invalid_data', 'Dati utente mancanti o non validi', ['status' => 400]);
        }
        ...
        $enabled = sanitize_text_field($user['enabled'] ?? '');

        if (!$username || !$email || !$first_name || !$last_name || !$enabled) {
            return new WP_Error('missing_fields', 'Campi obbligatori mancanti', ['status' => 422]);
        }

        try {
            $this->repository->update_user($user_id, $username, $email, $first_name, $last_name, $enabled);
            return new WP_REST_Response(['success' => true], 200);
        } catch (Exception $e) {
    ...
    public function adjust_user($user) {
    ...
    public function get_event_for_logged_user_enrich_players($players, int $user_id) {
